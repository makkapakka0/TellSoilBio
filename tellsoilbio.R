...
df_CN<-df[,c('TC','TOC','TN','LOI','amountC','amountN','C_N_Ratio','d13Ccorr',
             'dNcorr','ITM_x','ITM_y')]

coords <- df_CN[,c("ITM_x","ITM_y")]
...
Moran_test <- function(df, colname, lw){
...
cols <- names(df_CN)[!names(df_CN) %in% c("ITM_x","ITM_y")]

results<-data.frame(var=character(0),p_value=numeric(0))

for(col in cols){
  
  Moran_results <- Moran_test(df_CN, col, lw)
  
  result<-data.frame(var=col,p_value=as.numeric(Moran_results$p.value))
  
  results<-rbind(results,result)
  
}

write.csv(results,'D:\\tellsoilbio\\fulldata\\moran_CN.csv')


df<-read.csv('D:\\tellsoilbio\\fulldata\\fulldata.csv')


df_CN<-df[,c('LanduseSimplified','TC','TOC','TN','LOI','amountC','amountN',
             'd13Ccorr','dNcorr','C_N_Ratio','ITM_x','ITM_y')]

comparisons <- combn(unique(df_CN$LanduseSimplified), 2, simplify = FALSE)

sig_comparisons <- lapply(comparisons, function(pair) {
  group1 <- df_CN[df_CN$LanduseSimplified == pair[1], "TC"]
  group2 <- df_CN[df_CN$LanduseSimplified == pair[2], "TC"]
  p <- wilcox.test(group1, group2)$p.value
  if (p < 0.05) return(pair) else return(NULL)
}) %>% purrr::compact() 

ggplot(df_CN, aes(x = LanduseSimplified, y = TC, fill = LanduseSimplified)) +
  geom_boxplot(width=0.8,size=0.3) +
  stat_compare_means(comparisons = sig_comparisons, 
                     method = "wilcox.test", 
                     label = "p.signif",
  ...
  ggsave('D:\\tellsoilbio\\maps\\TC_stat.tiff',dpi=300,height=8,width=3.5)
...
sig_comparisons <- lapply(comparisons, function(pair) {
  group1 <- df[df$LanduseSimplified == pair[1], "TOC"]
  group2 <- df[df$LanduseSimplified == pair[2], "TOC"]
  p <- wilcox.test(group1, group2)$p.value
  if (p < 0.05) return(pair) else return(NULL)
}) %>% purrr::compact() 

ggplot(df_CN, aes(x = LanduseSimplified, y = TOC, fill = LanduseSimplified)) +
  geom_boxplot(width=0.8,size=0.3) +
  stat_compare_means(comparisons = sig_comparisons, 
                     method = "wilcox.test", 
                     label = "p.signif",
  ...
  ggsave('D:\\tellsoilbio\\maps\\TOC_stat.tiff',dpi=300,height=8,width=3.5)


comparisons <- combn(unique(df_CN$LanduseSimplified), 2, simplify = FALSE)

sig_comparisons <- lapply(comparisons, function(pair) {
  group1 <- df_CN[df_CN$LanduseSimplified == pair[1], "TN"]
  group2 <- df_CN[df_CN$LanduseSimplified == pair[2], "TN"]
  p <- wilcox.test(group1, group2)$p.value
  if (p < 0.05) return(pair) else return(NULL)
}) %>% purrr::compact() 

ggplot(df_CN, aes(x = LanduseSimplified, y = TN, fill = LanduseSimplified)) +
  geom_boxplot(width=0.8,size=0.3) +
  stat_compare_means(comparisons = sig_comparisons, 
                     method = "wilcox.test", 
                     label = "p.signif",
  ...
  ggsave('D:\\tellsoilbio\\maps\\TN_stat.tiff',dpi=300,height=8,width=3.5)

comparisons <- combn(unique(df_CN$LanduseSimplified), 2, simplify = FALSE)

sig_comparisons <- lapply(comparisons, function(pair) {
  group1 <- df_CN[df_CN$LanduseSimplified == pair[1], "LOI"]
  group2 <- df_CN[df_CN$LanduseSimplified == pair[2], "LOI"]
  p <- wilcox.test(group1, group2)$p.value
  if (p < 0.05) return(pair) else return(NULL)
}) %>% purrr::compact() 

ggplot(df_CN, aes(x = LanduseSimplified, y = LOI, fill = LanduseSimplified)) +
  geom_boxplot(width=0.8,size=0.3) +
  stat_compare_means(comparisons = sig_comparisons, 
                     method = "wilcox.test", 
                     label = "p.signif",
  ...
  ggsave('D:\\tellsoilbio\\maps\\LOI_stat.tiff',dpi=300,height=8,width=3.5)


comparisons <- combn(unique(df_CN$LanduseSimplified), 2, simplify = FALSE)

sig_comparisons <- lapply(comparisons, function(pair) {
  group1 <- df_CN[df_CN$LanduseSimplified == pair[1], "amountC"]
  group2 <- df_CN[df_CN$LanduseSimplified == pair[2], "amountC"]
  p <- wilcox.test(group1, group2)$p.value
  if (p < 0.05) return(pair) else return(NULL)
}) %>% purrr::compact() 

ggplot(df_CN, aes(x = LanduseSimplified, y = amountC, fill = LanduseSimplified)) +
  geom_boxplot(width=0.8,size=0.3) +
  stat_compare_means(comparisons = sig_comparisons, 
                     method = "wilcox.test", 
                     label = "p.signif",
  ...
  ggsave('D:\\tellsoilbio\\maps\\amountC_stat.tiff',dpi=300,height=8,width=3.5)


comparisons <- combn(unique(df_CN$LanduseSimplified), 2, simplify = FALSE)

sig_comparisons <- lapply(comparisons, function(pair) {
  group1 <- df_CN[df_CN$LanduseSimplified == pair[1], "amountN"]
  group2 <- df_CN[df_CN$LanduseSimplified == pair[2], "amountN"]
  p <- wilcox.test(group1, group2)$p.value
  if (p < 0.05) return(pair) else return(NULL)
}) %>% purrr::compact() 

ggplot(df_CN, aes(x = LanduseSimplified, y = amountN, fill = LanduseSimplified)) +
  geom_boxplot(width=0.8,size=0.3) +
  stat_compare_means(comparisons = sig_comparisons, 
                     method = "wilcox.test", 
                     label = "p.signif",
  ...
  ggsave('D:\\tellsoilbio\\maps\\amountN_stat.tiff',dpi=300,height=8,width=3.5)

comparisons <- combn(unique(df_CN$LanduseSimplified), 2, simplify = FALSE)

sig_comparisons <- lapply(comparisons, function(pair) {
  group1 <- df_CN[df_CN$LanduseSimplified == pair[1], "C_N_Ratio"]
  group2 <- df_CN[df_CN$LanduseSimplified == pair[2], "C_N_Ratio"]
  p <- wilcox.test(group1, group2)$p.value
  if (p < 0.05) return(pair) else return(NULL)
}) %>% purrr::compact() 

ggplot(df_CN, aes(x = LanduseSimplified, y = C_N_Ratio, fill = LanduseSimplified)) +
  geom_boxplot(width=0.8,size=0.3) +
  stat_compare_means(comparisons = sig_comparisons, 
                     method = "wilcox.test", 
                     label = "p.signif",
  ...
  ggsave('D:\\tellsoilbio\\maps\\C_N_Ratio_stat.tiff',dpi=300,height=8,width=3.5)

df_13C<-df_CN[df_CN$d13Ccorr>-50 & df_CN$d13Ccorr<0,]

comparisons <- combn(unique(df_CN$LanduseSimplified), 2, simplify = FALSE)

sig_comparisons <- lapply(comparisons, function(pair) {
  group1 <- df_13C[df_13C$LanduseSimplified == pair[1], "d13Ccorr"]
  group2 <- df_13C[df_13C$LanduseSimplified == pair[2], "d13Ccorr"]
  p <- wilcox.test(group1, group2)$p.value
  if (p < 0.05) return(pair) else return(NULL)
}) %>% purrr::compact() 

ggplot(df_13C, aes(x = LanduseSimplified, y = d13Ccorr, fill = LanduseSimplified)) +
  geom_boxplot(width=0.8,size=0.3) +
  stat_compare_means(comparisons = sig_comparisons, 
                     method = "wilcox.test", 
                     label = "p.signif",
  ...
  ggsave('D:\\tellsoilbio\\maps\\d13Ccorr_stat.tiff',dpi=300,height=8,width=3.5)


comparisons <- combn(unique(df_CN$LanduseSimplified), 2, simplify = FALSE)

sig_comparisons <- lapply(comparisons, function(pair) {
  group1 <- df_CN[df_CN$LanduseSimplified == pair[1], "dNcorr"]
  group2 <- df_CN[df_CN$LanduseSimplified == pair[2], "dNcorr"]
  p <- wilcox.test(group1, group2)$p.value
  if (p < 0.05) return(pair) else return(NULL)
}) %>% purrr::compact() 

ggplot(df_CN, aes(x = LanduseSimplified, y = dNcorr, fill = LanduseSimplified)) +
  geom_boxplot(width=0.8,size=0.3) +
  stat_compare_means(comparisons = sig_comparisons, 
                     method = "wilcox.test", 
                     label = "p.signif",
  ...
  ggsave('D:\\tellsoilbio\\maps\\dNcorr_stat.tiff',dpi=300,height=8,width=3.5)
